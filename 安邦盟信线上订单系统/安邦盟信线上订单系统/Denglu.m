...
@interface Denglu ()<UITextFieldDelegate>
...
- (void)viewDidLoad {
    ...
    self.xluser.delegate = self;
    self.xlmima.delegate = self;
...
-(BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    if (textField == self.xluser) {
        
        int len = (int)self.xluser.text.length;
        if (len > 10 && string.length >0) {
            if (![self isMobileNumberClassification:self.xluser.text]) {
    ...
    else if (textField == self.xlmima){
        
        int len = (int)self.xlmima.text.length;
        if (len >10 && string.length > 0) {
            if (![self validatePass:self.xlmima.text]) {
...
- (IBAction)denglu:(UIButton *)sender {
    ...
    else if(![self validatePass:self.xlmima.text])
    {
    ...
    else
    {
        ...
        NSDictionary *data = [NSDictionary dictionaryWithObjectsAndKeys:self.xluser.text,@"loginName",/*密码加密*/[MD5_sdk md5HexDigest: _xlmima.text],@"password", nil];
         ...
         *  放在接口返回成功的地方
         */
        Chaxun*chaxun=[[UIStoryboard storyboardWithName:@"Main" bundle:nil] instantiateViewControllerWithIdentifier:@"chaxun"];
